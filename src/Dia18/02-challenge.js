...
class Product {
    // No debes editar esta clase Product ❌
...
class Cart {
    ...
    deleteProduct(product) {
        const index = this.products.findIndex((element) => element === product);
        this.products.splice(index, 1);
    }
    calculateTotal() {
        const priceTotalProduct = this.products.map((product) => product.price * product.quantity);
        const sumaProducts = priceTotalProduct.reduce((acumulador, product) => acumulador + product);
        return sumaProducts;
    }
    getProducts() {
